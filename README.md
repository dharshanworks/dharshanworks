<div align="center">

<img src="https://capsule-render.vercel.app/api?type=venom&height=280&text=Dharshan%20R&fontSize=55&fontColor=ffffff&animation=fadeIn&color=0:0F2027,50:203A43,100:2C5364"/>

### Full-Stack Developer (MERN) • AWS & DevOps Enthusiast

<img src="https://readme-typing-svg.herokuapp.com?font=JetBrains+Mono&size=22&duration=3200&pause=1000&center=true&vCenter=true&width=900&lines=Building+Scalable+Web+Applications;Learning+Cloud+Native+Technologies;Exploring+AWS+and+DevOps;Passionate+About+Software+Engineering;Always+Learning+Something+New+🚀"/>

<p>
<img src="https://komarev.com/ghpvc/?username=dharshanworks&style=for-the-badge&color=blue"/>
<img src="https://img.shields.io/github/followers/dharshanworks?style=for-the-badge"/>
</p>

<p>
<a href="https://linkedin.com/in/iamdharsharnt">
<img src="https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin"/>
</a>

<a href="mailto:dharshanworks@gmail.com">
<img src="https://img.shields.io/badge/Gmail-EA4335?style=for-the-badge&logo=gmail"/>
</a>

<a href="https://leetcode.com/u/iamdharsharnt/">
<img src="https://img.shields.io/badge/LeetCode-FFA116?style=for-the-badge&logo=leetcode&logoColor=white"/>
</a>
</p>

**Open to Software Engineering, Full-Stack Development, Cloud & DevOps Internship Opportunities.**

</div>

---

# 👨‍💻 About Me

```yaml
Name: Dharshan R

Education:
  B.Tech Information Technology

Interests:
  - Software Engineering
  - Full-Stack Development
  - Cloud Computing
  - DevOps Engineering

Current Focus:
  - Building CloudForge
  - Enhancing CloudCart
  - Learning Kubernetes
  - Infrastructure as Code with Terraform
  - CI/CD Automation using Jenkins

Mission:
  Build scalable, reliable and cloud-ready applications
  while continuously improving engineering skills.
```

---

# 🚀 What I Do

✔ Build full-stack applications using the MERN stack

✔ Design and develop REST APIs

✔ Work with databases including MongoDB and MySQL

✔ Containerize applications using Docker

✔ Learn and implement cloud-native development practices

✔ Explore AWS and DevOps tooling for modern deployments

✔ Practice software engineering fundamentals and system design

---

# ⚙️ Technology Stack

### Languages

<p>
<img src="https://skillicons.dev/icons?i=java,js,python"/>
</p>

### Frontend

<p>
<img src="https://skillicons.dev/icons?i=react,html,css,tailwind"/>
</p>

### Backend

<p>
<img src="https://skillicons.dev/icons?i=nodejs,express"/>
</p>

### Database

<p>
<img src="https://skillicons.dev/icons?i=mongodb,mysql"/>
</p>

### Cloud & DevOps

<p>
<img src="https://skillicons.dev/icons?i=aws,docker,kubernetes,jenkins,terraform,linux,git,github"/>
</p>

### Tools

<p>
<img src="https://skillicons.dev/icons?i=vscode,postman"/>
</p>

---

# 🎯 Skill Areas

### Software Development

* Java
* JavaScript
* React.js
* Node.js
* Express.js
* MongoDB
* MySQL

### Cloud & DevOps

* AWS Fundamentals
* Linux
* Docker
* Kubernetes
* Jenkins
* Terraform
* Git & GitHub

### Engineering Practices

* REST API Development
* Database Design
* SDLC Fundamentals
* CI/CD Concepts
* Version Control

---

# 🏆 Featured Projects

## ☁️ CloudForge

Cloud-native project platform focused on modern application development and deployment practices.

**Tech Stack:** React, Node.js, Express, MongoDB, Docker

🔗 Repository: https://github.com/dharshanworks/CloudForge

---

## 🛒 CloudCart

Full-stack e-commerce platform featuring authentication, product management, shopping cart and checkout workflows.

**Tech Stack:** MERN Stack

🔗 Repository: Add Repository Link

---

## 🏥 Hospital Management System

Web-based application for managing patients, appointments and healthcare workflows.

**Tech Stack:** MERN Stack

---

## 💰 Compensation & Pay Equity System

Application designed to manage and analyze compensation structures.

**Tech Stack:** Full Stack Development

---

# 📜 Certifications & Learning

* AWS Cloud Practitioner (Learning Path)
* Docker Fundamentals
* Kubernetes Fundamentals
* Terraform Fundamentals
* Jenkins CI/CD Fundamentals

---

# 🎯 Current Goals

* [ ] Complete a production-style Kubernetes deployment
* [ ] Strengthen AWS architecture knowledge
* [ ] Build Terraform-based infrastructure projects
* [ ] Contribute to open source
* [ ] Secure a Software Engineering role
* [ ] Advance Cloud & DevOps skills

---

# 📊 GitHub Analytics

<p align="center">
<img width="48%" src="https://github-readme-stats.vercel.app/api?username=dharshanworks&show_icons=true&theme=tokyonight&hide_border=true"/>
<img width="48%" src="https://github-readme-stats.vercel.app/api/top-langs/?username=dharshanworks&layout=compact&theme=tokyonight&hide_border=true"/>
</p>

<p align="center">
<img width="98%" src="https://github-readme-streak-stats.herokuapp.com/?user=dharshanworks&theme=tokyonight&hide_border=true"/>
</p>

---

# 💡 Engineering Philosophy

> Build. Learn. Automate. Improve. Repeat.

---

# 🌐 Connect With Me

<p align="center">

<a href="https://linkedin.com/in/iamdharsharnt">
<img src="https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin"/>
</a>

<a href="mailto:dharshanworks@gmail.com">
<img src="https://img.shields.io/badge/Gmail-EA4335?style=for-the-badge&logo=gmail"/>
</a>

<a href="https://leetcode.com/u/iamdharsharnt/">
<img src="https://img.shields.io/badge/LeetCode-FFA116?style=for-the-badge&logo=leetcode&logoColor=white"/>
</a>

</p>

<div align="center">

### ⚡ Build • Ship • Scale • Automate ⚡

<img src="https://capsule-render.vercel.app/api?type=waving&section=footer&height=150&color=0:0F2027,50:203A43,100:2C5364"/>

</div>
